...
test_name = path.join(dir_path, "new_toefl.txt") # file with TOEFL dataset
data_file_name = path.join(dir_path, "lemmatized.text") # file with the text corpus

for dimension in [1000, 2000, 4000, 10000]:
    zero_vector = np.zeros(dimension)
    amount_dictionary = {}
    ...
    while line != "":
            if line != "\n":
                ...
                #words = lines[2] #ISSUE IS HERE, where it limits to 2. 
    ...
    print("Dimension: " + str(dimension) + " Percentage of correct answers: " + str(100 * right_answers / number_of_tests) + "%")
